...
export default function Reports() {
  ...
  function load(silent = false) { if (!silent) setLoading(true); return api(`/admin/reports?${query.toString()}`).then((data) => setReports(data.reports)).catch(() => navigate('/admin/login')).finally(() => { if (!silent) setLoading(false); }); }
  useEffect(() => { load(); const interval = window.setInterval(() => load(true), 15000); return () => window.clearInterval(interval); }, [filters.from, filters.to, filters.search]);
