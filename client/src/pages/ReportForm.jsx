...
export default function ReportForm() {
  ...
  function renderQuestion(question) {
    ...
    if (question.inputType === 'boolean') return <Field key={question.key} label={question.label} hint={hint}><div className="segmented-control">{['Yes', 'No'].map((option) => <button type="button" key={option} className={value === option || value === (option === 'Yes') ? 'is-selected' : ''} onClick={() => updateQuestion(question.key, option)}>{option}</button>)}</div></Field>;
    if (question.inputType === 'paceRating') return <Field key={question.key} label={question.label} hint={hint} className="field--full"><div className="pace-control">{paceOptions.map((pace) => <button type="button" className={value === pace ? `pace-control__option is-${pace.toLowerCase()}` : 'pace-control__option'} key={pace} onClick={() => updateQuestion(question.key, pace)}><span />{pace}</button>)}</div></Field>;
    if (question.inputType === 'select') return <Field key={question.key} label={question.label} hint={hint}><LedgerPicker ariaLabel={question.label} emptyLabel="No options are configured." onChange={(selectedValue) => updateQuestion(question.key, selectedValue)} options={(question.options || []).map((option) => typeof option === 'string' ? { value: option, label: option } : ({ value: option.value, label: option.label }))} placeholder="Choose an option" value={value || ''} /></Field>;
