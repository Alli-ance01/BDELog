...
export default function ReportForm() {
  ...
  function renderQuestion(question) {
    ...
    if (question.inputType === 'accountDetails') {
      const countKey = validation.countQuestionKey || 'accountsOpened';
      const count = Math.max(0, Number(answerFor(countKey)) || 0);
      const details = Array.isArray(value) ? value : [];
      const rows = Array.from({ length: count }, (_, index) => details[index] || { name: '', accountNumber: '' });
      function updateDetail(index, key, nextValue) {
        const next = rows.map((entry, rowIndex) => rowIndex === index ? { ...entry, [key]: nextValue } : entry);
        updateQuestion(question.key, next);
      }
      return <Field key={question.key} label={`${question.label} (${count})`} hint={hint || 'One separate row is required for every account opened today.'} className="field--full" asLabel={false}><div className="account-details"><p className="account-details__hint">The row count follows “Accounts opened today”. Enter the account holder name and the matching 10-digit account number.</p>{count === 0 ? <p className="account-details__empty">Enter the number of accounts opened today above to reveal the detail rows.</p> : rows.map((entry, index) => <div className="account-details__row" key={`${question.key}-${index}`}><span className="account-details__index">{String(index + 1).padStart(2, '0')}</span><label><span className="field__label">Account holder name</span><input type="text" value={entry.name} onChange={(event) => updateDetail(index, 'name', event.target.value)} placeholder="Full name" required={required || count > 0} /></label><label><span className="field__label">Account number</span><input type="text" inputMode="numeric" pattern="[0-9]{10}" maxLength="10" value={entry.accountNumber} onChange={(event) => updateDetail(index, 'accountNumber', event.target.value.replace(/\D/g, '').slice(0, 10))} placeholder="10 digits" required={required || count > 0} /></label></div>)}</div></Field>;
    }
